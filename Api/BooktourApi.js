...
router2.post("/tourbooked", fetchuser, async (req, res) => {
  const { name, email, phone, adults, childs, tourId, title, description, img, price, stayTime, city } = req.body;
  try {
    ...
    tour = new BookTour({ name, email, phone, adults, childs, user: req.user.userId, tourId, title, description, img, price, stayTime, city });
    const savedTour = await tour.save();
    res.json({ savedTour, user: req.user.userId });
  } catch (error) {
    return res
      .status(400)
      .json({ error: "Tour Couldn't be booked, please try again!", error: error.message });
  }
});
